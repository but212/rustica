...
//! ## Quick Start
//!
//! An `IsoLens` combines an `Iso` with lens-style access and updates. A common representation
//! maps a structure to `(focus, context)` so updates can preserve unrelated fields:
//!
//! ```rust
...
//!     fn forward(&self, person: &Person) -> (String, Person) {
//!         (person.name.clone(), person.clone())
//!     }
//!     fn backward(&self, value: &(String, Person)) -> Person {
//!         Person { name: value.0.clone(), ..value.1.clone() }
//!     }
//! }
//!
...
//! ```
//!
//! Lens laws and composition behavior are covered by the datatype tests.
//!
//! See also: [`crate::datatypes::lens`] and [`crate::traits::iso::Iso`].
//!
