...
pub trait Foldable<T: ReturnTypeConstraints> {

    /// Left-associative fold of a structure.
    fn fold_left<U: ReturnTypeConstraints, F: FnTrait<(U, T), U>>(self, init: U, f: F) -> U;

    /// Right-associative fold of a structure.
    fn fold_right<U: ReturnTypeConstraints, F: FnTrait<(T, U), U>>(self, init: U, f: F) -> U;

    /// Maps elements to a monoid and combines them.
    fn fold_map<M: Monoid + ReturnTypeConstraints, F: FnTrait<T, M>>(self, f: F) -> M;
...
impl<T: ReturnTypeConstraints> Foldable<T> for Vec<T> {
    fn fold_left<U: ReturnTypeConstraints, F: FnTrait<(U, T), U>>(self, init: U, f: F) -> U {
...
impl<T: ReturnTypeConstraints> Foldable<T> for Option<T> {
    fn fold_left<U: ReturnTypeConstraints, F: FnTrait<(U, T), U>>(self, init: U, f: F) -> U {
        match self {
