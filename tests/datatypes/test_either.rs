...
fn test_either_applicative() {
    ...
    // R lift R lift R = R
    assert_eq!(
        a_right.lift3(&b_right, &c_right, |x, y, z| x * y + z).unwrap_right(),
        10
    );
    // L lift R lift R = L (a)
    assert_eq!(
        a_left.lift3(&b_right, &c_right, |x, y, z| x * y + z).unwrap_left(),
        "a error"
    );
    // R lift L lift R = L (b)
    assert_eq!(
        a_right.lift3(&b_left, &c_right, |x, y, z| x * y + z).unwrap_left(),
        "b error"
    );
    // R lift R lift L = L (c)
    assert_eq!(
        a_right.lift3(&b_right, &c_left, |x, y, z| x * y + z).unwrap_left(),
        "c error"
    );
    // L lift L lift R = L (a)
    assert_eq!(
        a_left.lift3(&b_left, &c_right, |x, y, z| x * y + z).unwrap_left(),
        "a error"
    );
