mod test_choice {
    ...
    fn test_choice_empty_and_len() {
        ...
        let single = Choice::new(1, vec![]);
        assert!(!single.is_empty(), "Single element Choice should not be empty");
        assert_eq!(single.len(), 1, "Single element Choice should have length 1");

        let multi = Choice::new(1, vec![2, 3]);
        assert!(!multi.is_empty(), "Multi element Choice should not be empty");
