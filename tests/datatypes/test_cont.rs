...
fn test_cont_creation_and_run() {
    ...
    let cont = Cont::new(|k: Arc<dyn Fn(i32) -> i32>| k(42));
    let result = cont.run(|x| x * 2);
    assert_eq!(result, 84);
}
...
fn test_cont_bind() {
    let cont = Cont::pure(42);
    let bound = cont.bind(Arc::new(|x| Cont::pure(x + 1)));
...
#[test]
fn test_cont_apply() {
    let cont_val = Cont::pure(42);
    let cont_fn = Cont::pure(Arc::new(|x: i32| x + 1) as Arc<dyn Fn(i32) -> i32 + Send + Sync>);
...
fn test_cont_complex_composition() {
    ...
    let initial = Cont::pure(10);
    let step1 = initial.fmap(|x| x * 2);  // 20
    let step2 = step1.bind(Arc::new(|x| Cont::pure(x + 5))); // 25
...
fn test_cont_error_handling() {
    ...
    let safe_div = |n: i32, d: i32| -> Cont<String, i32> {
        if d == 0 {
            Cont::new(|_| "Division by zero".to_string())
        } else {
            Cont::pure(n / d)
        }
    };
