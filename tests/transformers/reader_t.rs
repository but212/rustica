...
fn test_reader_t_composition() {
    ...
    let composed: ReaderT<(i32, String), Option<i32>, i32> = ReaderT::new(move |env: (i32, String)| {
...
fn test_reader_t_with_complex_error_handling() {
    ...
    let process_value: ReaderT<Config, Result<i32, String>, i32> = 
        ReaderT::new(|cfg: Config| {
            if cfg.debug_mode {
    ...
    let valid_config = Config { debug_mode: false, max_value: 10 };
    let result = validate_config.try_run_reader(valid_config.clone());
    assert!(result.is_ok());
    assert_eq!(result.unwrap(), true);

    // Test validation with an invalid config
    let invalid_config = Config { debug_mode: true, max_value: 0 };
    ...
    let odd_config = Config { debug_mode: false, max_value: 7 };
    let result = process_value.try_run_reader(odd_config);
    assert!(result.is_err());
    assert_eq!(result.unwrap_err().message(), &"Cannot process odd max values");

    // Test with context for better error reporting
    let result = process_value.try_run_reader_with_context(valid_config, "performing number processing");
    assert!(result.is_ok());
